...
class SchedulerService:
    ...
    def __init__(self, app=None):
        ...
        check_days_str = os.getenv('CHECK_DAYS', '0,1,2,3,4,5,6')
        self.check_days = [
            int(day.strip())
            for day in check_days_str.split(',')
            if day.strip()
        ]
        ...
        self.always_send_summary = (
            os.getenv('ALWAYS_SEND_SUMMARY', 'false').lower() == 'true'
        )
        ...
        try:
            ...
            with self.app.app_context():
                ...
                if today.weekday() not in self.check_days:
                    print(
                        f"Skipping check for {today.strftime('%A')} "
                        "(not in check days)"
                    )
                    return
                ...
                print("📅 Report generated for:", today.date())
                ...
                if not receipts:
                    print(
                        f"No receipts found for {today.strftime('%Y-%m-%d')}"
                    )

                    if self.alert_recipients:
                        print("Sending no-receipts alert email...")
                        success, error = (
                            self.email_service.send_no_receipts_alert(
                                self.alert_recipients,
                                today
                            )
                        )
                        ...
                        if success:
                            print(
                                f"Alert sent successfully to "
                                f"{len(self.alert_recipients)} recipients"
                            )
                        else:
                    ...
                    print(
                        f"Found {len(receipts)} receipts for "
                    ...
                    if self.always_send_summary and self.alert_recipients:
                        ...
                        if success:
                            ...
                            print("❌ Failed to send summary:", error)

        except Exception as e:
            print("❌ Error in check_daily_receipts:", str(e))

    def _calculate_summary(self, receipts):
        """Calculate summary statistics from receipts"""
        total_receipts = len(receipts)
        total_revenue = sum(
            float(receipt.gross_amount) for receipt in receipts
        )
    ...
    def start(self):
        ...
        if not self.alert_recipients:
            print(
                ...
                "Scheduler will not send alerts."
            )
            return
        ...
        print(
            f"✅ Scheduler started. Daily check scheduled for "
            f"{self.check_hour:02d}:{self.check_minute:02d} "
            f"on days: {self.check_days}"
        )
    ...
    def trigger_check_now(self):
        ...
        print("▶️ Manually triggering daily receipt check...")
